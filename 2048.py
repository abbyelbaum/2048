...
class gamePlan(tk.Frame):
    ...
    """ Starts game by making grid and filling random 2 cells with 2s """
    def start(self):
    ...
    """ When two neighboring tiles are not the same, this is called to stack them next to each other """
    def stack(self):
    ...
    """ When two neighboring tiles are the same, this is called to merge them into one tile with double its value """
    def merge(self):
        for i in range(4):
            for j in range(3):
    ...
    """ Reconfigures board reversed in temporary variable and copies back over into main grid (helper) """
    def backwards(self):
        temp = []
        for i in range(4):
            temp.append([])
    ...
    """ Reconfigures board by pushing tiles over, uses temporary board and copies into main grid (helper) """
    def translate(self):
        temp = [[0] * 4 for _ in range(4)]
        for i in range(4):
            for j in range(4):
    ...
    """ Every turn, this is called to randomly add a 2 or 4 to the grid """
    def add(self):
        r = random.randint(0,3)
        c = random.randint(0,3)
    ...
    """ The next two methods check if moves are possible (horizontal or vertical) """
    def h_real(self):
        for i in range(4):
            for j in range(3):
    ...
    """ Figures out if player won or lost """
    def end(self):
